...
func startRTMPServer(onVideoSample func(sample media.Sample) error) {
...
func (h *Handler) OnPublish(streamContext *rtmp.StreamContext, timestamp uint32, cmd *rtmpmsg.NetStreamPublish) error {
...
func (h *Handler) OnVideo(timestamp uint32, payload io.Reader) error {
	...
	if h.lastTimestamp > 0 {
		// RTMP时间戳单位是毫秒，转换为纳秒
		timeDiff := int64(timestamp-h.lastTimestamp) * int64(time.Millisecond)
		if timeDiff > 0 {
			duration = time.Duration(timeDiff)
		} else {
			// 如果时间戳没有变化，使用默认值
	...
	return h.onVideoSample(media.Sample{
		Data:     outBuf,
		Duration: duration,
	})
}
